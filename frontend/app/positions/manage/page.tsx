...
export default function ManageEmployeePositionsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const employeesResponse = await fetch('http://localhost:8003/employees/unassigned');
  ...
  const updateEmployeePosition = async () => {
    ...
    try {
      setUpdating(true);
      const response = await fetch(`http://localhost:8003/employees/${editingEmployee}/position`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Ошибка при обновлении должности сотрудника');
      }
